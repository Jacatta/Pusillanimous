...
public class knoblBehavior : MonoBehaviour {
	...
	void Update () {
        ...
        {
            this.transform.position = Camera.main.ScreenToWorldPoint(new Vector3(StartScreenP.x, StartScreenP.y + (BarMax * 4*  (BB.distance / BB.FinishLine)), StartScreenP.z));
    ...
    public void StartUp(BackgroundBehaviors BB)
    {
        ...
        SurfLine.transform.position = Camera.main.ScreenToWorldPoint(new Vector3(StartScreenP.x, StartScreenP.y + (BarDiff * 4 * (BB.SurfaceLine / BB.FinishLine)), StartScreenP.z));
