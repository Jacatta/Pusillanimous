...
public class pause : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Debug.Log("results: " + Results.name);

        //ResultsBack.enabled = false;


        Begin = GameObject.Find("Canvas_OnStart").GetComponent<Canvas>();
        Begin.enabled = false;
        /*
        Results = GameObject.Find("Canvas_Results");//.GetComponent<Canvas>();
    ...
    public void OnPause()
    {
        ...
        else
        {
            ...
            LastSceneSpeed = Background.sceneSpeed;
            Background.sceneSpeed = 0;
        }
    }
    ...
    public void OnSlowPause(float i)
    {
        ...
        else
        {
           ...
           // StartCoroutine(TaperBGSpeed((Background.sceneSpeed / 5)));
            Background.sceneSpeed = (Background.sceneSpeed/5);
        }
    }
    ...
    public IEnumerator TaperBGSpeed(float inc, float waitTime, float EndSpeed)
    {
        ...
        if (Background.sceneSpeed > (EndSpeed))
        {
            
            while (Background.sceneSpeed > (EndSpeed))
            {
                Background.sceneSpeed += -(inc);
                yield return new WaitForSeconds(waitTime);
            }
        }
        else if (Background.sceneSpeed < (EndSpeed))
        {
            while (Background.sceneSpeed < (EndSpeed))
            {
                Background.sceneSpeed += inc;
